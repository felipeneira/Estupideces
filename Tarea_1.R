...
t <- length(datos$r_us)
sigma2 = (t(e)%*%e)/(t-5-1)
MV <- t/2*log(2*3.14*sigma2)-(1/2*sigma2)*sum(e^2)
Ygorro <- B[1]-B[2]*X[,2]-B[3]*X[,3]-B[4]*X[,4]-B[5]*X[,5]-B[6]*X[,6]

RA <- solve(t(X)%*%X)%*%(t(X)%*%e)

suma <- colSums(e)

R2 <- ((t(e)%*%e)/((t(Y-Ygorro))%*%(Y-Ygorro)))
prueba <- lm(data = datos, Y ~ r_us+ r_ch + cobre + petroleo + broad_dollar)
